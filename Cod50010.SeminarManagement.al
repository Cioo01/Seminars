Codeunit 50010 "Seminar Management"
{
    ...
    var
        ...
        SalesNRecivablesSetup: Record "Sales & Receivables Setup";
        LineN0: Integer;
        LineCreated: Boolean;
    begin
        SalesSetup.Get();
        SalesSetup.TestField("G/L Account No.");
        //TODO: sprawdź czy rejestracja ma ustawiony Stan na "zakończone" 
        //(faktury mają być wystawiane tylko dla zakończonych rejestracji)
        ...
        //TODO: ogranicz przetwarzane SRL do krotek z zaznaczonym polem 
        //"Do fakturowania" (faktury mają być wystawiane tylko dla uczestników z zaznaczonym polem "Do fakturowania")
        SeminarRegLine.SetRange("To Invoice", true);

        //TODO: ogranicz przetwarzane SRL do krotek,
        // które nie miały już wcześniej wystawionych faktur
        // (uczestnicy, którzy mieli już wystawine faktury, nie mają ich ponownie wystawianych)
        ...
        if SeminarRegLine.FindSet() then begin
            ...
            repeat
                ...
                SalesLine.Validate("No.", SalesNRecivablesSetup."G/L Account No.");
                ...
                //TODO: wprowadź dwa pola wymagane w opisie wiersza faktury 'Seminar ''%1'' participant ''%2''';
                SalesLine.Description := StrSubstNo(DescriptionTemplateTxt, PSeminarRegHeader."Seminar Name", SeminarRegLine."Participant Name");
    ...
    var
        InvoiceError: Label 'Cant issue an invoice when state isnt finished';
}
